        ...
        function BlogNavigationImgAdd(edit) {
            var index = layui.layer.open({
                ...
                success: function (layero, index) {
                    ...
                    if (edit) {
                        body.find(".lableId").val(edit.NavigationId);  //站点Id
                        body.find(".lableName").val(edit.NavigationName);  //站点名称
        ...
        table.on('tool(blogNavigationImgList)', function (obj) {
            ...
            if (layEvent === 'edit') { //编辑
                ...
                layer.confirm('确定删除此轮播图？', { icon: 3, title: '提示信息' }, function (index) {
                    $.post("/Blog/BlogNavigationImg/DeleteBlogNavigationImg", {
                        blogNavigationImgId: data.NavigationId  //将需要删除的NavigationId作为参数传入
                    }, function (res) {
                        layer.msg(res.RetMsg);
                        layer.close(index);
                        if (res.RetCode == 200) {
